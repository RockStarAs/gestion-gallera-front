...
import { eliminarCuidoAPI, listarAllCuidos, listarCuidosGalloAPI } from '@/api/cuidoApi';
...
export default{
    ...
    methods : {
        ...
        async listarDatos(){
            const peticion = await listarAllCuidos(this.gallo.id_gallo);
        ...
        abriModalCuido(){
            // modalCuidado
            this.gallo = {...datosIniciales};
            this.$refs.modalCuidado.abrirModalAgregar(this.gallo);
        },
        async abrirModalCuidoModificar(cuido){
            await this.buscarRegistroGallo(cuido.id_gallo);
            this.$refs.modalCuidado.abrirModalModificar(this.gallo,{...cuido});
        },
        async abrirModalObservarCuido(cuido){
            await this.buscarRegistroGallo(cuido.id_gallo);
